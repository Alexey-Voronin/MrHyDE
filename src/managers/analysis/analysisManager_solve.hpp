...
void AnalysisManager::ROLSolve()
{
  ...
  const std::string hessVecMode = generalROL2.get("HessVec Mode", "exact");
  const bool vectorContractCheck = generalROL2.get("vector contract check", false);
...
MrHyDE_OptVector AnalysisManager::recoverParametersFromFile() {
  ...
  // Wire Riesz operators: prefer H(curl), fall back to mass.
  if (!params_->hcurlForwardOp.is_null() && !params_->hcurlInvOperator.is_null()) {
    newparams.setMassOperators(params_->hcurlForwardOp, params_->hcurlInvOperator);
    newparams.setProperDualSpaces(true);
  } else if (!params_->massForwardOp.is_null() && !params_->massInvOperator.is_null()) {
