...
def make_plot(
    ...
    # Assign a single marker per refinement tag (r1, r2, r3, ...) so the
    # same mesh refinement is comparable across scaling modes.
    marker_by_tag: Dict[str, str] = {}
    for label in sorted(data.keys()):
        match = re.search(r"_r(\d+)_", label)
        if not match:
            continue
        tag = match.group(0)
        if tag not in marker_by_tag:
            marker_by_tag[tag] = SERIES_MARKERS[len(marker_by_tag) % len(SERIES_MARKERS)]
    ...
    for series_idx, label in enumerate(sorted(data.keys())):
        ...
        tag_match = re.search(r"_r(\d+)_", label)
        marker = marker_by_tag.get(tag_match.group(0)) if tag_match else SERIES_MARKERS[series_idx % len(SERIES_MARKERS)]
